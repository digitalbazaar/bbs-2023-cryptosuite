...
export async function createVerifier({verificationMethod}) {
  const key = await Bls12381Multikey.from(verificationMethod, {
    algorithm: Bls12381Multikey.ALGORITHMS.BBS_BLS12381_SHAKE256
  });
  // FIXME: just return `verifier`, it already implements BBS `multiverify`
...
// FIXME: remove / adapt to use BBS multiverify
async function _multiverify({verifier, data} = {}) {
  ...
  const localKeyPair = await Bls12381Multikey.from({publicKeyMultibase});
